...
public class UIWidget : UIRect
{
				...
				if(isAnchoredVertically)
				{
					if(bottomAnchor.target != null && topAnchor.target != null)
					{
						...
						else
						{
							...
							diff = diff - (diff & 1);
							if(diff != 0)
								NGUIMath.AdjustWidget(this, 0f, -diff * 0.5f, 0f, diff * 0.5f);
						}
					}
	...
	public virtual bool isVisible { get { return mIsVisibleByPanel && mIsVisibleByAlpha && mIsInFront && finalAlpha > 0.001f && NGUITools.GetActive(this); } }
	...
	public Bounds CalculateBounds(Transform relativeParent)
	{
		...
		else
		{
			...
			for(int j = 1; j < 4; ++j)
				b.Encapsulate(toLocal.MultiplyPoint3x4(corners[j]));
			return b;
		}
	}
	...
	public virtual void MarkAsChanged()
	{
		if(NGUITools.GetActive(this))
		{
			...
			if(panel != null && enabled && NGUITools.GetActive(gameObject) && !mPlayMode)
			{
				...
				if(material != null)
					NGUITools.SetDirty(panel.gameObject);
#endif
			}
		}
	}
	...
	private void SetDepth()
	{
		if(panel != null)
		{
			mDepth = (int)(-(transform.position.z - panel.transform.position.z));
		}
	}
	...
	public void CheckDepth()
	{
		if(panel != null)
		{
			int nDepth = (int)(-(transform.position.z - panel.transform.position.z));
			if(nDepth != depth)
			{
				depth = nDepth;
			}
